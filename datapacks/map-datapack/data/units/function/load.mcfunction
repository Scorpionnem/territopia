scoreboard objectives add unit.health dummy
scoreboard objectives add unit.attack dummy
scoreboard objectives add unit.attackrange dummy
scoreboard objectives add unit.movementrange dummy
scoreboard objectives add unit.id dummy
scoreboard objectives add unit.select dummy

scoreboard objectives add unit.moovesleft dummy
scoreboard objectives add unit.maxmooves dummy

scoreboard objectives add unit.savemovementrange dummy
scoreboard objectives add unit.boatlevel dummy
scoreboard objectives add unit.canclimbmountains dummy

scoreboard objectives add radius dummy
scoreboard players set 2 radius 2

scoreboard objectives add swapplaces dummy

scoreboard objectives add scheduleid dummy
scoreboard objectives add thistileid dummy

scoreboard objectives add blockinteraction dummy