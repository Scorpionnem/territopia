scoreboard objectives add currentplayer dummy

scoreboard objectives add carrot minecraft.used:minecraft.carrot_on_a_stick

scoreboard objectives add turn dummy

scoreboard objectives add stars dummy

scoreboard objectives add score dummy

bossbar add player1 "player1"
bossbar add player2 "player2"
bossbar add player3 "player3"
bossbar add player4 "player4"

scoreboard objectives add village.id dummy
scoreboard objectives add village.maxunitscount dummy
scoreboard objectives add village.unitscount dummy
scoreboard objectives add village.starsperturn dummy
scoreboard objectives add village.idplayer dummy
scoreboard objectives add village.level dummy
scoreboard objectives add village.xp dummy
scoreboard objectives add village.xp.nextlevel dummy



scoreboard objectives add xp.timer dummy